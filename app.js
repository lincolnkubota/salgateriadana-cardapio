...
function render(){
  ...
  document.getElementById('products').innerHTML=items.map(item=>{const photo=photoFor(item.name,item.category);const imageClass=item.category==='Pratos Rápidos'?' prato-rapido-photo':'';return `<article class="product"><div class="food${imageClass} ${photo?'':'no-photo'}" ${photo?`style="background-image:url('${photo}')" role="img" aria-label="Foto ilustrativa de ${item.name}"`:''}></div><div class="product-content"><p class="category">${item.category}</p><h3>${item.name}</h3><p class="description">${item.description}</p><strong>${money(item.price)}</strong></div></article>`}).join('');
